...
def mindmap():
    ...
    FS=17;ROW=44;TOP=46;GAP=70;PADW=10
    ...
    root_w=tw(root["t"],20)+44; colx={0:40,1:40+root_w+GAP}
    ...
    def conn(n):
        ...
        for k in n["children"]:
            ...
            conn(k)
    conn(root)
    L.append('<rect x="40" y="%.1f" width="%.1f" height="50" rx="11" fill="#1e293b"/>'%(rc-25,root_w))
    L.append('<text x="%.1f" y="%.1f" text-anchor="middle" fill="#fff" font-size="20" font-weight="700">%s</text>'%(40+root_w/2,rc+7,root["t"]))
    for n in alln:
